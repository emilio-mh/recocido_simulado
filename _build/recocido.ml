...
Arg.parse[
  ...
  ("-ti"  , Arg.Float  (function i -> temp_i  := i), "Temperatura inicial"                    );
  ...
  ("-n"   , Arg.Int    (function i -> n       := i), "Númro de evaluaciones"                  );
...
begin
  exec db (sprintf "SELECT * FROM connections WHERE id_city_1 IN (%s) AND id_city_2 IN (%s)" str str)
    ~cb:(fun row _ ->
...
let promedio = !sum /. (float_of_int (!e_s - 1)) in
...
let costo (s : int array) : float = 
  ...
  !w /. (promedio *. (float_of_int (!e_s + 1))) in
...
let rec eval (n : int) =
  if n = 0 then
    ...
    let out = open_out (sprintf "reporte%d.tsp" !semilla) in
